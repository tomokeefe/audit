...
export async function scrapeWithFirecrawl(url: string) {
  ...
  try {
    ...
    if (!response.data || !response.data.success) {
      throw new Error(
        `Firecrawl returned unsuccessful response: ${JSON.stringify(response.data)}`,
      );
    }
    ...
    console.log(
      `   ✓ Content length: ${html?.length || 0} bytes HTML, ${markdown?.length || 0} bytes Markdown`,
    );

    if (!html || html.length < 100) {
      throw new Error(
        `Firecrawl returned insufficient content (${html?.length || 0} bytes)`,
      );
    }
    ...
    console.log(
      `   ✓ Metadata: ${data.metadata?.ogTitle ? "OG tags ✓" : "No OG tags"}, ${data.metadata?.keywords ? "Keywords ✓" : "No keywords"}`,
    );
