...
export async function scrapeWithScraperAPI(url: string) {
  ...
  try {
    ...
    console.log(
      `   ⏳ Requesting page through ScraperAPI with rendering (may take 10-30s)...`,
    );
    ...
    if (!response.data || response.data.length < 100) {
      throw new Error(
        `ScraperAPI returned insufficient content (${response.data.length} bytes)`,
      );
    }
  } catch (renderError) {
    ...
    if (statusCode === 500 && renderMode) {
      console.warn(
        `   ⚠️  render=true failed with 500, trying without rendering...`,
      );
      try {
        ...
        const elapsed = Date.now() - startTime;
        console.log(
          `   ✓ ScraperAPI (no render) response received in ${elapsed}ms`,
        );
        console.log(`   ✓ Content length: ${response.data.length} bytes`);
        renderMode = false;
      } catch (noRenderError) {
  ...
  try {
    if (!response || !response.data || response.data.length < 100) {
      throw new Error(
        `ScraperAPI returned insufficient content (${response?.data?.length || 0} bytes)`,
      );
    }
    ...
    console.log(
      `   ✓ Extracted: Title="${title.slice(0, 50)}...", ${headings.length} headings, ${paragraphs.length} paragraphs`,
    );
    ...
    if (statusCode === 401 || statusCode === 403) {
      ...
      console.error(
        `   ⚠️  ScraperAPI server error (500) - their service may be having issues`,
      );
      console.error(`   Response data:`, responseData);
      console.error(
        `   Recommendation: Try without render parameter or contact ScraperAPI support`,
      );
    } else if (statusCode) {
