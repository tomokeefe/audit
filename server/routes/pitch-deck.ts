...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (
      ...
      file.originalname.match(/\.(ppt|pptx|pdf)$/i)
    ) {
      cb(null, true);
    } else {
...
async function extractFromPowerPoint(filePath: string): Promise<string> {
  try {
    ...
    throw new Error(
      "Failed to extract text from PowerPoint file. Please ensure the file is a valid .ppt or .pptx file.",
    );
  }
}
...
export const handlePitchDeckAudit: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    console.log(
      "[PITCH DECK] Extracted text preview:",
    ...
    console.log(
      "[PITCH DECK] Sections count:",
      auditResult.sections?.length || 0,
    );
    ...
    res.status(500).json({
      error:
        error instanceof Error
          ? error.message
          : "Failed to process pitch deck audit",
    });
  }
};
