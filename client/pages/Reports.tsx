...
export default function Reports() {
  ...
  const filteredAudits = audits
    .filter((audit) => {
      ...
      switch (timeRange) {
        ...
        case "30days":
          return (
            now.getTime() - auditDate.getTime() <= 30 * 24 * 60 * 60 * 1000
          );
        case "90days":
          return (
            now.getTime() - auditDate.getTime() <= 90 * 24 * 60 * 60 * 1000
          );
        default:
          return true;
      }
  ...
  const getReportAnalytics = () => {
    ...
    const averageScore = Math.round(
      filteredAudits.reduce((sum, audit) => sum + audit.overallScore, 0) /
        totalAudits,
    );

    // Performance distribution
    const excellent = filteredAudits.filter((a) => a.overallScore >= 80).length;
    const good = filteredAudits.filter(
      (a) => a.overallScore >= 60 && a.overallScore < 80,
    ).length;
    const needsImprovement = filteredAudits.filter(
      (a) => a.overallScore < 60,
    ).length;

    // Monthly trend data
    const monthlyData = filteredAudits.reduce(
      (acc, audit) => {
        const month = new Date(audit.date).toLocaleDateString("en-US", {
          year: "numeric",
          month: "short",
        });
        if (!acc[month]) {
    ...
    const scoreRanges = [
      {
        range: "90-100",
        count: filteredAudits.filter((a) => a.overallScore >= 90).length,
      },
      {
        range: "80-89",
        count: filteredAudits.filter(
          (a) => a.overallScore >= 80 && a.overallScore < 90,
        ).length,
      },
      {
        range: "70-79",
        count: filteredAudits.filter(
          (a) => a.overallScore >= 70 && a.overallScore < 80,
        ).length,
      },
      {
        range: "60-69",
        count: filteredAudits.filter(
          (a) => a.overallScore >= 60 && a.overallScore < 70,
        ).length,
      },
      {
        range: "50-59",
        count: filteredAudits.filter(
          (a) => a.overallScore >= 50 && a.overallScore < 60,
        ).length,
      },
      {
        range: "0-49",
        count: filteredAudits.filter((a) => a.overallScore < 50).length,
      },
    ];
    ...
    if (recentAudits.length > 0 && olderAudits.length > 0) {
      const recentAvg =
        recentAudits.reduce((sum, a) => sum + a.overallScore, 0) /
        recentAudits.length;
      const olderAvg =
        olderAudits.reduce((sum, a) => sum + a.overallScore, 0) /
        olderAudits.length;
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Average scores and audit count by month
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="h-80">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Number of audits in each score range
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="h-80">
            ...
            <Card className="mb-8">
              <CardHeader>
                ...
                <CardDescription>
                  Key insights from your audit data
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Detailed view of recent audit results
                </CardDescription>
              </CardHeader>
              <CardContent>
